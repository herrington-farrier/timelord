...
Timelord packs **productive** time only. `dayMinutes` splits into three sections. Even by default (`floor(dayMinutes / 3)`, remainder on evening), or set directly on Strategize → Day, where the three must add back to Day Length — Day Length is the truth, and a split left over from a different one is ignored rather than allowed to contradict it. Personal (Morning Routine, Break, Evening Routine) pauses productivity and is not in weekly capacity. Assignable week hours = `dayMinutes × 7`.
