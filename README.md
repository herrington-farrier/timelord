...
- **Strategize** (`/edit`) — day length and timer sound/vibrate, Respawn, weekly hours, Events date ranges, lists grouped by bucket, duration-only appointments
...
Design and packing rules live in [docs/logic-map.md](docs/logic-map.md), the Firestore shape in [docs/data-model.md](docs/data-model.md), deferred work in [docs/todo.md](docs/todo.md), the manual check list in [docs/verify.md](docs/verify.md), and — if you ever fork this or fold it into something larger — [docs/porting.md](docs/porting.md).
